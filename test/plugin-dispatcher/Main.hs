...
newPluginSpec = do
  describe "New plugin dispatcher operation" $
    it "dispatches response correctly" $ do
      ...
      let req0 = GReq 0 "0" Nothing Nothing                          (Just $ IdInt 0) (\_ -> return () :: IO ())         $ return $ IdeResultOk $ T.pack "text0"
          req1 = GReq 1 "1" Nothing Nothing                          (Just $ IdInt 1) defCallback $ return $ IdeResultOk $ T.pack "text1"
          req2 = GReq 2 "2" Nothing Nothing                          (Just $ IdInt 2) delayedCallback      $ return      $ IdeResultOk $ T.pack "text2"
          req3 = GReq 3 "3" Nothing (Just (filePathToUri "test", 2)) Nothing          defCallback $ return $ IdeResultOk $ T.pack "text3"
          req4 = GReq 4 "4" Nothing Nothing                          (Just $ IdInt 3) defCallback $ return $ IdeResultOk $ T.pack "text4"
